@extends('layouts.main')

@section('content')
    <!-- Contact Form Section Begin -->
    <section class="contact-from-section spad" style="margin-top:50px" style="padding-top:0px">
        <div class="container">
            <div class="row">
                <div class="col-lg-12">
                    <div class="section-title">
                        <h2>FGD MBKM</h2>
                        {{-- <p>Fill in the form below to register.</p> --}}
                    </div>
                </div>
            </div>
            <div class="row justify-content-center">
                <div class="col-lg-12">
                    <div class="card" style="border: 0px">
                        <div class="card-body">
                            {{-- CONTENT --}}
                            <div style="text-align:center">
                                <h5 style="font-weight: 600">MBKM Management Group Discussion Forum
                                    <br>(Forum Group Discussion: Independent Learning- Independent Campus)
                                </h5>
                                <a href="https://forms.gle/3qugvzsKQiqQt5RT9" class="primary-btn my-3">Register for this
                                    event</a>
                            </div>
                            <br>
                            <div>
                                <h5 style="font-weight: 600">OVERVIEW</h5>
                                <p style="text-align: justify; text-indent: 5%">
                                    The Study Program is encouraged to implement the Independent Learning and Campus Freedom
                                    (MBKM) Policy by the Ministry of Education and Culture, by establishing agreements in
                                    the
                                    form of cooperation documents (MoUs/SPK) with partners, including several state
                                    universities, government agencies, and the business world. The forms of cooperative
                                    activities include education implementation, internships in the business and industrial
                                    sectors, research, and community service such as rural development, similar to Community
                                    Service Learning (KKN). In the effort to optimize additional competencies for students
                                    to
                                    compete globally, the forms of collaboration need to be developed and enhanced further.
                                    FGD for the Development of MBKM Management as a recognition of student activities
                                    towards
                                    strengthening the University's IKU 3 is in line with the goal of producing graduates
                                    with
                                    competencies relevant to the needs of the business/industrial world. The FGD on the
                                    Development of MBKM Management pertains to the efforts of developing the MBKM
                                    curriculum,
                                    FGD on Curriculum and Learning Tool Development, tailored to the expertise of each
                                    speaker,
                                    which, of course, corresponds to the study program. The focus of the MBKM management
                                    focus
                                    group discussion revolves around the skills required by Chemistry Study Program
                                    graduates.
                                    Strengthening the curriculum of the Chemistry Study Program cannot be separated from the
                                    support, evaluation, and guidance of the Chemical Association, Indonesian Chemical
                                    Society
                                    (HKI). Additionally, it is essential to receive reinforcement from international
                                    chemists
                                    (foreign countries) to gain insights/benchmarking from Chemistry Departments at
                                    world-class
                                    campuses. This activity is part of realizing excellent human resources, where the
                                    transformation of higher education plays a crucial role. One of these roles involves
                                    developing and strengthening the partnership implementation of MBKM to produce more
                                    talents
                                    capable of competing on a global scale. This will have an impact on the development of
                                    resilient human resources capable of thinking and acting extraordinarily, having the
                                    ability
                                    to quickly adapt to face challenges, not falling behind, and succeeding in competition.
                                    This
                                    activity will also have an impact on other study programs within the scope of
                                    Universitas
                                    Jambi.
                                </p>
                            </div>

                            <div>
                                <h5 style="font-weight: 600">ACTIVITY OBJECTIVES </h5>
                                <span>
                                    There are several objectives to be achieved through this activity, including:
                                    <div style="margin-left: 3%">
                                        <ol type="1">
                                            <li>Establishment of a sustainable partnership network with accomplished human
                                                resources from
                                                Chemistry Departments at the national and international levels.</li>
                                            <li>Forging collaboration between the Chemistry Study Program, Faculty of
                                                Science
                                                and
                                                Technology, Universitas Jambi, and Chemistry Study Programs from other
                                                universities.
                                            </li>
                                            <li>
                                                Conducting a Forum Group Discussion on the Management of the Independent
                                                Learning
                                                and Campus Freedom (MBKM) Program Study.
                                            </li>
                                        </ol>
                                    </div>
                                </span>
                            </div>
                            <br>
                            <div>
                                <h5 style="font-weight: 600">ACTIVITY IMPLEMENTERS </h5>
                                <span> Several Lecturers and Educators who will be assigned in the implementation of the
                                    Independent Learning and Campus Freedom (MBKM) Program Study Forum Group Discussion,
                                    attached agenda.</span>
                            </div>
                            <br>
                            <div>
                                <h5 style="font-weight: 600">VENUE AND TIME OF IMPLEMENTATION </h5>
                                <span>
                                    The Independent Learning and Campus Freedom (MBKM) Program Study Forum Group Discussion
                                    will
                                    be held at Swiss-Bell Hotel, Jambi, on Wednesday, November 15th, 2023.
                                </span>
                            </div>
                            <br>
                            <div>
                                <h5 style="font-weight: 600">MBKM Management FGD
                                    <br> Facilitator/Speaker
                                </h5>
                                <span>
                                    <div style="margin-left: 3%">
                                        <ol type="1">
                                            <li>
                                                Prof. Drs. H. Sutrisno, M.Sc., Ph.D (Rector of Universitas Jambi)

                                            </li>
                                            <li>
                                                Dr. Sukro Muhab, M.Si. (President of HKI)

                                            </li>
                                            <li>
                                                Prof. Anang WM Diah, M.Si., Ph.D

                                            </li>
                                        </ol>
                                    </div>
                                </span>
                            </div>
                            <br>
                            <div>
                                <h5 style="font-weight: 600">PARTICIPANTS</h5>
                                <span>
                                    We invite academics from various National and International Universities.
                                </span>
                            </div>
                            <div>
                                <div class="container mt-2">
                                    <div class="row">
                                        <div class="col">
                                            <table class="table table-bordered">
                                                <thead>
                                                    <tr>
                                                        <th>No</th>
                                                        <th>List of Universities</th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    <tr>
                                                        <td>1</td>
                                                        <td>Universitas Gadjah Mada</td>
                                                    </tr>
                                                    <tr>
                                                        <td>2</td>
                                                        <td>Universitas Indonesia</td>
                                                    </tr>
                                                    <tr>
                                                        <td>3</td>
                                                        <td>Universitas Diponegoro</td>
                                                    </tr>
                                                    <tr>
                                                        <td>4</td>
                                                        <td>Universitas Brawijaya</td>
                                                    </tr>
                                                    <tr>
                                                        <td>5</td>
                                                        <td>Universitas Airlangga</td>
                                                    </tr>
                                                    <tr>
                                                        <td>6</td>
                                                        <td>Universitas Negeri Padang</td>
                                                    </tr>
                                                    <tr>
                                                        <td>7</td>
                                                        <td>Institut Teknologi Bandung</td>
                                                    </tr>
                                                    <tr>
                                                        <td>8</td>
                                                        <td>Institut Pertanian Bogor</td>
                                                    </tr>
                                                    <tr>
                                                        <td>9</td>
                                                        <td>Institut Teknologi Sepuluh November</td>
                                                    </tr>
                                                    <tr>
                                                        <td>10</td>
                                                        <td>Universitas Padjajaran</td>
                                                    </tr>
                                                    <tr>
                                                        <td>11</td>
                                                        <td>Universitas Andalas</td>
                                                    </tr>
                                                    <tr>
                                                        <td>12</td>
                                                        <td>Universitas Negeri Medan</td>
                                                    </tr>
                                                    <tr>
                                                        <td>13</td>
                                                        <td>Universitas Pendidikan Indonesia</td>
                                                    </tr>
                                                    <tr>
                                                        <td>14</td>
                                                        <td>Universitas Sumatera Utara</td>
                                                    </tr>
                                                    <tr>
                                                        <td>15</td>
                                                        <td>Universitas Syah Kuala</td>
                                                    </tr>
                                                    <tr>
                                                        <td>16</td>
                                                        <td>Universitas Bengkulu</td>
                                                    </tr>
                                                    <tr>
                                                        <td>17</td>
                                                        <td>Universitas Sriwijaya</td>
                                                    </tr>
                                                    <tr>
                                                        <td>18</td>
                                                        <td>Universitas Riau</td>
                                                    </tr>
                                                    <tr>
                                                        <td>19</td>
                                                        <td>Universitas Lampung</td>
                                                    </tr>
                                                    <tr>
                                                        <td>20</td>
                                                        <td>Universitas Negeri Semarang</td>
                                                    </tr>
                                                    <tr>
                                                        <td>21</td>
                                                        <td>Universitas Negeri Malang</td>
                                                    </tr>
                                                    <tr>
                                                        <td>22</td>
                                                        <td>Universitas Negeri Surabaya</td>
                                                    </tr>
                                                    <tr>
                                                        <td>23</td>
                                                        <td>Universitas Negeri Semarang</td>
                                                    </tr>
                                                    <tr>
                                                        <td>24</td>
                                                        <td>Universitas Negeri Yogyakarta</td>
                                                    </tr>
                                                    <tr>
                                                        <td>25</td>
                                                        <td>Universitas Negeri Manado</td>
                                                    </tr>
                                                </tbody>
                                            </table>
                                        </div>
                                        <div class="col">
                                            <table class="table table-bordered">
                                                <thead>
                                                    <tr>
                                                        <th>No</th>
                                                        <th>List of Universities</th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    <tr>
                                                        <td>26</td>
                                                        <td>Universitas Palangkaraya</td>
                                                    </tr>
                                                    <tr>
                                                        <td>27</td>
                                                        <td>Universitas Jambi</td>
                                                    </tr>
                                                    <tr>
                                                        <td>28</td>
                                                        <td>Universitas Islam Negeri Sultan Thaha Saifuddin</td>
                                                    </tr>
                                                    <tr>
                                                        <td>29</td>
                                                        <td>Universitas Jember</td>
                                                    </tr>
                                                    <tr>
                                                        <td>30</td>
                                                        <td>Universitas Muhammadiyah Jakarta</td>
                                                    </tr>
                                                    <tr>
                                                        <td>31</td>
                                                        <td>Universitas Hasanuddin</td>
                                                    </tr>
                                                    <tr>
                                                        <td>32</td>
                                                        <td>Universitas Sultan Ageng Tirtayasa</td>
                                                    </tr>
                                                    <tr>
                                                        <td>33</td>
                                                        <td>Universitas Halu Oleo</td>
                                                    </tr>
                                                    <tr>
                                                        <td>34</td>
                                                        <td>Universitas Negeri Jakarta</td>
                                                    </tr>
                                                    <tr>
                                                        <td>35</td>
                                                        <td>Universitas Mulawarman</td>
                                                    </tr>
                                                    <tr>
                                                        <td>36</td>
                                                        <td>Universitas Jenderal Soedirman</td>
                                                    </tr>
                                                    <tr>
                                                        <td>37</td>
                                                        <td>Universitas Negeri Manado</td>
                                                    </tr>
                                                    <tr>
                                                        <td>38</td>
                                                        <td>Universitas Nusa Cendana</td>
                                                    </tr>
                                                    <tr>
                                                        <td>39</td>
                                                        <td>Universitas Udayana</td>
                                                    </tr>
                                                    <tr>
                                                        <td>40</td>
                                                        <td>Universitas Sam Ratulangi</td>
                                                    </tr>
                                                    <tr>
                                                        <td>41</td>
                                                        <td>Universitas Lambung Mangkurat</td>
                                                    </tr>
                                                    <tr>
                                                        <td>42</td>
                                                        <td>Universitas Islam Negeri Sunan Kalijaga</td>
                                                    </tr>
                                                    <tr>
                                                        <td>43</td>
                                                        <td>Universitas Islam Indonesia</td>
                                                    </tr>
                                                    <tr>
                                                        <td>44</td>
                                                        <td>Universitas Bangka Belitung</td>
                                                    </tr>
                                                    <tr>
                                                        <td>45</td>
                                                        <td>Universitas Malikussaleh</td>
                                                    </tr>
                                                    <tr>
                                                        <td>46</td>
                                                        <td>Universitas Tanjungpura</td>
                                                    </tr>
                                                    <tr>
                                                        <td>47</td>
                                                        <td>Institut Teknologi Sumatera</td>
                                                    </tr>
                                                    <tr>
                                                        <td>48</td>
                                                        <td>UIN Syarif Hidayatullah, Jakarta</td>
                                                    </tr>
                                                    <tr>
                                                        <td>49</td>
                                                        <td>UIN Malik Maulana Ibrahim, Malang</td>
                                                    </tr>
                                                    <tr>
                                                        <td>50</td>
                                                        <td>Universitas Pertahanan</td>
                                                    </tr>
                                                </tbody>
                                            </table>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>
@endsection
